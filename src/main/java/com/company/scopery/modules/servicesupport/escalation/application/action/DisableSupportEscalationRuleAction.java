...
@Component
public class DisableSupportEscalationRuleAction {
    private final SupportEscalationRuleRepository repo; private final SupportAuthorizationService auth;
    public DisableSupportEscalationRuleAction(SupportEscalationRuleRepository repo, SupportAuthorizationService auth){ this.repo=repo; this.auth=auth; }
    @Transactional
    public SupportEscalationRuleResponse execute(UUID workspaceId, UUID ruleId) {
        ...
        return SupportEscalationRuleResponse.from(repo.save(rule.disable()));
    }
}
