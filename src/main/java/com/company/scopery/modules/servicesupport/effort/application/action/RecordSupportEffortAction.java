...
@Component
public class RecordSupportEffortAction {
    ...
    public RecordSupportEffortAction(SupportEffortRecordRepository efforts, SupportCaseRepository cases,
    ...
    @Transactional
    public SupportEffortResponse execute(UUID workspaceId, RecordSupportEffortCommand cmd) {
        auth.requireManage(workspaceId);
        var supportCase = cases.findById(cmd.caseId()).orElseThrow(() -> SupportExceptions.caseNotFound(cmd.caseId()));
        var saved = efforts.save(SupportEffortRecord.create(workspaceId, cmd.caseId(), cmd.resourceProfileId(), cmd.effortHours(), cmd.effortDate()));
        activity.logSuccess("SUPPORT_EFFORT", saved.id(), "SUPPORT_EFFORT_RECORDED", "Effort recorded " + saved.effortHours() + "h");
        boolean rebuild = supportCase.projectId() != null;
        if (rebuild) events.publishEvent(new ProfitabilityRebuildRequestedEvent(supportCase.projectId(), "SUPPORT_EFFORT_RECORDED"));
        return SupportEffortResponse.from(saved, rebuild);
    }
}
