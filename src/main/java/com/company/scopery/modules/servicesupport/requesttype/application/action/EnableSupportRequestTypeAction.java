...
@Component
public class EnableSupportRequestTypeAction {
    private final SupportRequestTypeRepository repo; private final SupportAuthorizationService auth;
    public EnableSupportRequestTypeAction(SupportRequestTypeRepository repo, SupportAuthorizationService auth){ this.repo=repo; this.auth=auth; }
    @Transactional
    public SupportRequestTypeResponse execute(UUID workspaceId, UUID requestTypeId) {
        ...
        return SupportRequestTypeResponse.from(repo.save(rt.enable()));
    }
}
