...
@Component
public class CloseIncidentAction {
    private final SupportIncidentRecordRepository repo; private final SupportAuthorizationService auth;
    public CloseIncidentAction(SupportIncidentRecordRepository repo, SupportAuthorizationService auth){ this.repo=repo; this.auth=auth; }
    @Transactional
    public SupportIncidentResponse execute(UUID workspaceId, UUID incidentId) {
        ...
        try { return SupportIncidentResponse.from(repo.save(incident.close())); }
