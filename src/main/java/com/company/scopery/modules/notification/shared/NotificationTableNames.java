package com.company.scopery.modules.notification.shared;

public final class NotificationTableNames {
    public static final String EMAIL_TEMPLATE         = "notification_email_template";
    public static final String EMAIL_TEMPLATE_VERSION = "notification_email_template_version";
    public static final String EMAIL_RULE             = "notification_email_rule";
    public static final String EMAIL_DELIVERY         = "notification_email_delivery";
    public static final String EMAIL_OUTBOX           = "notification_email_outbox";

    private NotificationTableNames() {}
}
