...
import com.company.scopery.modules.notification.advanced.alert.application.command.DismissAlertEventCommand;
...
@Component
public class DismissAlertEventAction {
    ...
    public DismissAlertEventAction(AlertEventRepository events, CurrentUserAuthorizationService currentUser, WorkspaceIamIntegrationService iam) {
        this.events=events; this.currentUser=currentUser; this.iam=iam;
    }
    @Transactional
    public AlertEventResponse execute(DismissAlertEventCommand c) {
        ...
        return AlertEventResponse.from(events.save(event.dismiss()));
    }
}
