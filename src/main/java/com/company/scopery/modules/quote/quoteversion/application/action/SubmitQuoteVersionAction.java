...
@Component
public class SubmitQuoteVersionAction {
    ...
    public SubmitQuoteVersionAction(ProjectRepository projects, QuoteVersionRepository versions,
    ...
    public QuoteVersionResponse execute(UUID projectId, UUID quoteId, UUID versionId) {
        authorization.requireSubmit(projectId);
        ...
        if (!version.isDraft()) throw QuoteExceptions.versionInvalidStatus(version.id(), "submit");
        var versionLines = lines.findByQuoteVersionId(version.id());
        if (versionLines.isEmpty()) throw QuoteExceptions.versionNoLines(version.id());
        var summary = summaries.findByQuoteVersionId(version.id())
                .orElseThrow(() -> QuoteExceptions.versionSummaryInvalid("Summary missing"));
        if (summary.totalQuotedAmount() == null || summary.totalQuotedAmount().signum() <= 0) {
            throw QuoteExceptions.submitValidationFailed("totalQuotedAmount must be > 0");
        }
        calculationService.validateDiscountReason(version.discountMethod(), version.discountPercent(),
                version.discountAmount(), summary.subtotalBeforeDiscount(), version.discountReason());
        if (version.validUntil() != null && version.validUntil().isBefore(java.time.LocalDate.now())) {
            throw QuoteExceptions.submitValidationFailed("validUntil is in the past");
        }
        version = versions.save(version.submit(actorId));
        publisher.enqueueVersion(version, "QUOTE_SUBMITTED");
        publisher.audit(AuditEventType.QUOTE_SUBMITTED, actorId, project, version, "SubmitQuoteVersionAction");
        activityLogger.logSuccess(QuoteEntityTypes.QUOTE_VERSION, version.id(),
                QuoteActivityActions.QUOTE_SUBMITTED, "QUOTE_SUBMITTED");
