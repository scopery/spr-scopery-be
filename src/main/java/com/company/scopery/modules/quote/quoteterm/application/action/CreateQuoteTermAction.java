...
@Component
public class CreateQuoteTermAction {
    ...
    public CreateQuoteTermAction(QuoteVersionRepository versions, QuoteTermRepository terms,
    ...
    @Transactional
    public QuoteTermResponse execute(UUID projectId, UUID quoteId, UUID versionId, String termType,
                                     String title, String content, Integer displayOrder, Boolean clientVisible) {
        authorization.requireTermCreate(projectId);
        ...
        QuoteTermType type = QuoteEnumParser.parseRequired(QuoteTermType.class, termType,
                QuoteErrorCatalog.QUOTE_TERM_CONTENT_REQUIRED.code(), "termType");
        int order = displayOrder == null ? terms.findByQuoteVersionId(versionId).size() : displayOrder;
        QuoteTerm term = terms.save(QuoteTerm.create(
                version.id(), projectId, type, title, content, order,
                clientVisible == null || clientVisible));
        publisher.enqueueVersion(version, "QUOTE_TERM_CREATED");
        activityLogger.logSuccess(QuoteEntityTypes.QUOTE_TERM, term.id(),
                QuoteActivityActions.QUOTE_TERM_CREATED, "Quote term created");
