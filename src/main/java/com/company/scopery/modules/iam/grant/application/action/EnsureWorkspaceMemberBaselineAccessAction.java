...
/**
 * Ensures every active workspace member has a usable baseline on the workspace IAM resource:
 * personal productivity + project/work viewing (and light contribute rights).
 *
 * Idempotent: creates a DIRECT grant when missing, otherwise attaches missing actions
 * onto an existing ALLOW grant (e.g. OWNER).
 */
@Component
public class EnsureWorkspaceMemberBaselineAccessAction {

    private static final Logger log = LoggerFactory.getLogger(EnsureWorkspaceMemberBaselineAccessAction.class);

    private record BaselineAction(String permissionCode, String actionCode) {}

    /**
     * Contributor baseline — enough to list/open projects and do day-to-day work.
     * Does not include invite/manage-member or archive/manage-project admin rights.
     */
    private static final List<BaselineAction> BASELINE_ACTIONS = List.of(
            // Personal productivity (inbox / nav / favorites)
            ...
            // Projects (list requires PROJECT_VIEW on workspace resource)
            ...
            // Reporting — match API action codes (not bare VIEW)
    ...
    public EnsureWorkspaceMemberBaselineAccessAction(
    ...
    private IamAccessGrant createDirectGrant(IamAuthResource resource, UUID userId) {
        IamAccessGrant created = grantRepository.save(IamAccessGrant.createWithMetadata(
                ...
                null,
                null,
                "Workspace member baseline access",
                userId));
        log.info("Created DIRECT baseline grant {} for user {} on workspace resource {}",
