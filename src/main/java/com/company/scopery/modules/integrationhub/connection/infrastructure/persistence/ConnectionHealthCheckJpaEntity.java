package com.company.scopery.modules.integrationhub.connection.infrastructure.persistence;
import com.company.scopery.common.audit.AuditableJpaEntity;
import com.company.scopery.modules.integrationhub.shared.constant.IntegrationTableNames;
import jakarta.persistence.*; import lombok.Getter; import lombok.NoArgsConstructor; import lombok.Setter;
import java.time.Instant; import java.util.UUID;
@Entity @Table(name = IntegrationTableNames.HEALTH_CHECK) @Getter @Setter @NoArgsConstructor
public class ConnectionHealthCheckJpaEntity extends AuditableJpaEntity {
    @Id private UUID id;
    @Column(name="workspace_id", nullable=false) private UUID workspaceId;
    @Column(name="connection_id", nullable=false) private UUID connectionId;
    @Column(name="health_status", nullable=false) private String healthStatus;
    @Column(name="checked_at", nullable=false) private Instant checkedAt;
    @Column(name="duration_ms") private Long durationMs;
    @Column(columnDefinition="text") private String message;
    @Column(name="error_code") private String errorCode;
    @Version private Integer version;
}
