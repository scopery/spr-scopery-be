...
@Component
public class ValidateImportJobAction {
    private final ImportJobRepository jobs;
    private final IntegrationAuthorizationService auth;
    private final IntegrationActivityLogger activity;
    public ValidateImportJobAction(ImportJobRepository jobs, IntegrationAuthorizationService auth, IntegrationActivityLogger activity) {
        this.jobs = jobs; this.auth = auth; this.activity = activity;
    }
    @Transactional
    public ImportJobResponse execute(UUID workspaceId, UUID jobId, long totalRows, long validRows, long invalidRows) {
        ...
        var saved = jobs.save(j.markValidated(totalRows, validRows, invalidRows));
        activity.logSuccess("INTEGRATION_IMPORT_JOB", saved.id(), "INTEGRATION_IMPORT_JOB_VALIDATED", "Import job validated");
