...
@Component
public class RetryDeadLetterEventAction {
    ...
    public RetryDeadLetterEventAction(DeadLetterEventRepository repo, IntegrationAuthorizationService auth, IntegrationActivityLogger activity) {
    ...
    public DeadLetterEventResponse execute(UUID workspaceId, UUID deadLetterId) {
        ...
        var saved = repo.save(d.retry());
        activity.logSuccess("INTEGRATION_DEAD_LETTER", saved.id(), "INTEGRATION_DEAD_LETTER_RETRIED", "Dead letter event retried");
