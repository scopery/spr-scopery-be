...
@Component
public class ResolveSyncConflictAction {
    ...
    public ResolveSyncConflictAction(SyncConflictRepository conflicts, IntegrationAuthorizationService auth, IntegrationActivityLogger activity) {
        this.conflicts = conflicts; this.auth = auth; this.activity = activity;
    }
    @Transactional
    public SyncConflictResponse execute(UUID workspaceId, UUID conflictId, String strategy, String notes) {
        ...
        var saved = conflicts.save(c.resolve(strategy, notes, null));
        activity.logSuccess("INTEGRATION_SYNC_CONFLICT", saved.id(), "INTEGRATION_SYNC_CONFLICT_RESOLVED", "Sync conflict resolved: " + strategy);
