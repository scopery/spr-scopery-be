...
@Component
public class ActivateDocumentTypeFieldAction {
    ...
    public ActivateDocumentTypeFieldAction(DocumentTypeFieldRepository fieldRepository,
    ...
    public DocumentTypeFieldResponse execute(UUID documentTypeId, UUID fieldId) {
        ...
        DocumentTypeField saved = fieldRepository.save(field.activate());
        activityLogger.logSuccess(KnowledgeEntityTypes.DOCUMENT_TYPE_FIELD, saved.id(),
                KnowledgeActivityActions.ACTIVATE_DOCUMENT_TYPE_FIELD,
                "Document type field activated: " + saved.fieldKey().value());
        platformPublisher.enqueue(saved, "DOCUMENT_TYPE_FIELD_ACTIVATED");
