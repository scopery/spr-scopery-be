...
import com.company.scopery.modules.aiagent.execution.application.command.MarkExecutionFailedCommand;
...
@Component
public class MarkExecutionFailedAction {
    ...
    public MarkExecutionFailedAction(ExecutionLogRepository executionLogRepository,
    ...
    @Transactional
    public ExecutionLogDetailResponse execute(MarkExecutionFailedCommand command) {
        ...
        if (log.status() != ExecutionStatus.RUNNING) {
            throw AiAgentExceptions.invalidExecutionStatusTransition(log.status().name(), "FAILED");
        }
