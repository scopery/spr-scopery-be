...
import com.company.scopery.modules.aiagent.execution.api.request.ExecuteEventConfigRequest;
import com.company.scopery.modules.aiagent.execution.api.request.ExecuteEventRequest;
...
public class ExecutionController {
    ...
    public ResponseEntity<ApiResponse<ExecutionRunResponse>> executeEvent(
        ...
        ExecutionRunResponse response = executionApplicationService.executeEvent(command);
    ...
    public ResponseEntity<ApiResponse<ExecutionRunResponse>> executeEventConfig(
        ...
        ExecuteEventConfigCommand command = new ExecuteEventConfigCommand(
                ...
                request.triggerSource(),
                request.inputVariables());
        ExecutionRunResponse response = executionApplicationService.executeEventConfig(command);
        return ResponseEntity.ok(ApiResponse.success(response));
    }
}
