...
@Component
public class BulkCreateFunctionalItemCustomPropertyJobHandler implements BulkJobHandler {

    public static final String JOB_TYPE = "BULK_CREATE_FI_CUSTOM_PROPERTY";

    private static final Logger log = LoggerFactory.getLogger(BulkCreateFunctionalItemCustomPropertyJobHandler.class);

    private final BulkCreateFunctionalItemCustomPropertyAction action;
    private final BulkJobService bulkJobService;
    private final ObjectMapper objectMapper;

    public BulkCreateFunctionalItemCustomPropertyJobHandler(BulkCreateFunctionalItemCustomPropertyAction action,
    ...
    public void process(BulkJobRecord job) {
        ...
        try {
            ...
            BulkCreateFunctionalItemCustomPropertyCommand cmd = objectMapper.readValue(
                    job.payloadJson(), BulkCreateFunctionalItemCustomPropertyCommand.class);
            action.execute(cmd);
            bulkJobService.markSucceeded(job.id(), null);
        } catch (Exception e) {
            log.error("[BulkCreateFICustomPropertyJobHandler] jobId={} failed: {}", job.id(), e.getMessage());
