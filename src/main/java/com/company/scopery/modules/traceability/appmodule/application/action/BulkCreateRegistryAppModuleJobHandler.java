...
@Component
public class BulkCreateRegistryAppModuleJobHandler implements BulkJobHandler {

    public static final String JOB_TYPE = "BULK_CREATE_REGISTRY_APP_MODULE";

    private static final Logger log = LoggerFactory.getLogger(BulkCreateRegistryAppModuleJobHandler.class);

    private final BulkCreateRegistryAppModuleAction action;
    private final BulkJobService bulkJobService;
    private final ObjectMapper objectMapper;

    public BulkCreateRegistryAppModuleJobHandler(BulkCreateRegistryAppModuleAction action,
    ...
    public void process(BulkJobRecord job) {
        ...
        try {
            ...
            BulkCreateRegistryAppModuleCommand cmd = objectMapper.readValue(
                    job.payloadJson(), BulkCreateRegistryAppModuleCommand.class);
            action.execute(cmd);
            bulkJobService.markSucceeded(job.id(), null);
        } catch (Exception e) {
            log.error("[BulkCreateRegistryAppModuleJobHandler] jobId={} failed: {}", job.id(), e.getMessage());
