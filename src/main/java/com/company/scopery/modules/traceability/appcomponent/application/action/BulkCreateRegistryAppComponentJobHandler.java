...
@Component
public class BulkCreateRegistryAppComponentJobHandler implements BulkJobHandler {

    public static final String JOB_TYPE = "BULK_CREATE_REGISTRY_APP_COMPONENT";

    private static final Logger log = LoggerFactory.getLogger(BulkCreateRegistryAppComponentJobHandler.class);

    private final BulkCreateRegistryAppComponentAction action;
    private final BulkJobService bulkJobService;
    private final ObjectMapper objectMapper;

    public BulkCreateRegistryAppComponentJobHandler(BulkCreateRegistryAppComponentAction action,
    ...
    public void process(BulkJobRecord job) {
        ...
        try {
            ...
            BulkCreateRegistryAppComponentCommand cmd = objectMapper.readValue(
                    job.payloadJson(), BulkCreateRegistryAppComponentCommand.class);
            action.execute(cmd);
            bulkJobService.markSucceeded(job.id(), null);
        } catch (Exception e) {
            log.error("[BulkCreateRegistryAppComponentJobHandler] jobId={} failed: {}", job.id(), e.getMessage());
