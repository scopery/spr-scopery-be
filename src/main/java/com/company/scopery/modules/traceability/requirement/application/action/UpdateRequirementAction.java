...
public class UpdateRequirementAction {
    ...
    public UpdateRequirementAction(RequirementRepository repo, TraceabilityAuthorizationService authorization, TraceabilityActivityLogger activityLogger) {
    ...
    public RequirementResponse execute(UpdateRequirementCommand c) {
        ...
        RequirementPriority priority = c.priority() != null ? TraceabilityEnumParser.parseRequired(RequirementPriority.class, c.priority(), "priority") : null;
        RequirementType type = c.requirementType() != null ? TraceabilityEnumParser.parseRequired(RequirementType.class, c.requirementType(), "requirementType") : null;
        var saved = repo.save(req.update(c.title(), c.description(), priority, type, c.applicationId()));
        activityLogger.logSuccess(TraceabilityEntityTypes.REQUIREMENT, saved.id(), TraceabilityActivityActions.REQUIREMENT_UPDATED, "Requirement updated");
