...
public class RegistryDataEntityFieldPersistenceMapper {
    ...
    public RegistryDataEntityField toDomain(RegistryDataEntityFieldJpaEntity e) {
        return new RegistryDataEntityField(
                e.getId(), e.getEntityId(), e.getWorkspaceId(), e.getColumnName(), e.getDataType(),
                e.getMaxLength(), e.isNullable(), e.isUnique(), e.isPrimaryKey(),
                e.getDefaultValue(), e.getPrecision(), e.getScale(),
                e.getRemark(), e.getDisplayOrder(),
    ...
    public RegistryDataEntityFieldJpaEntity toJpaEntity(RegistryDataEntityField d) {
        ...
        e.setId(d.id()); e.setEntityId(d.entityId()); e.setWorkspaceId(d.workspaceId());
        e.setColumnName(d.columnName()); e.setDataType(d.dataType()); e.setMaxLength(d.maxLength());
        e.setNullable(d.isNullable()); e.setUnique(d.isUnique()); e.setPrimaryKey(d.isPrimaryKey());
        e.setDefaultValue(d.defaultValue()); e.setPrecision(d.precision()); e.setScale(d.scale());
        e.setRemark(d.remark()); e.setDisplayOrder(d.displayOrder()); e.setStatus(d.status().name());
        if (d.createdAt() != null) { e.setVersion(d.version()); e.setCreatedAt(d.createdAt()); }
        return e;
    }
}
