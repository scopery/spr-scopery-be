...
public class TraceLinkController {
    ...
    private final BatchCreateTraceLinkAction batchCreate;
    private final ArchiveTraceLinkAction archive;
    private final TraceLinkQueryService query;

    public TraceLinkController(CreateTraceLinkAction create, BatchCreateTraceLinkAction batchCreate,
                               ArchiveTraceLinkAction archive, TraceLinkQueryService query) {
        this.create = create;
    ...
    @PostMapping
    @Operation(summary = "Create trace link")
    public ApiResponse<TraceLinkResponse> create(@PathVariable UUID projectId,
                                                 @Valid @RequestBody CreateTraceLinkRequest r) {
        return ApiResponse.success(create.execute(
                new CreateTraceLinkCommand(projectId, r.sourceType(), r.sourceId(),
                        r.targetType(), r.targetId(), r.linkType())));
    }

    @PostMapping("/batch")
    @Operation(summary = "Batch create trace links (idempotent — skips existing)")
    public ApiResponse<BatchTraceLinkResponse> batchCreate(@PathVariable UUID projectId,
                                                           @Valid @RequestBody BatchCreateTraceLinkRequest r) {
        return ApiResponse.success(batchCreate.execute(projectId, r.links()));
    }

    @GetMapping
    @Operation(summary = "List trace links")
    public ApiResponse<List<TraceLinkResponse>> list(@PathVariable UUID projectId) {
        return ApiResponse.success(query.list(projectId));
    }

    @GetMapping("/{linkId}")
    @Operation(summary = "Get trace link")
    ...
    @PatchMapping("/{linkId}/archive")
    @Operation(summary = "Archive trace link")
