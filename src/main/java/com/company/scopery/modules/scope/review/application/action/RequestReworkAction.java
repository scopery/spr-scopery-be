...
@Component
public class RequestReworkAction {
    ...
    public RequestReworkAction(DeliverableRepository deliverables, DeliverableReviewRepository reviews,
    ...
    @Transactional
    public DeliverableReviewResponse execute(RequestReworkCommand command) {
        ...
        try {
            review = reviews.save(review.requestRework(actor.id(), command.reason()));
        ...
        try {
            deliverables.save(d.requestRework());
        } catch (IllegalStateException ex) {
            throw ScopeExceptions.invalidStatus(ex.getMessage());
        }
        activityLogger.logSuccess(ScopeEntityTypes.REVIEW, review.id(), ScopeActivityActions.DELIVERABLE_REWORK_REQUESTED,
                "Deliverable rework requested");
