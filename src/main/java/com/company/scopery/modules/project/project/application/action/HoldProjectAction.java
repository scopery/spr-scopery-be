...
@Component
public class HoldProjectAction {

    private final ProjectRepository projectRepository;
    private final ProjectActivityLogger activityLogger;
    private final ProjectWorkspaceAuthorizationService authorizationService;

    public HoldProjectAction(ProjectRepository projectRepository,
    ...
    public ProjectResponse execute(HoldProjectCommand command) {
        ...
        if (project.status() != ProjectStatus.ACTIVE) {
            throw ProjectExceptions.projectCannotHold(command.id());
        }

        Project held = project.hold();
        Project saved = projectRepository.save(held);

        activityLogger.logSuccess(
                ProjectEntityTypes.PROJECT,
                saved.id(),
                ProjectActivityActions.HOLD_PROJECT,
                "Project put on hold: " + saved.code()
        );
