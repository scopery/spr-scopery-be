...
import com.company.scopery.modules.project.task.application.command.ReopenTaskCommand;
...
public class ReopenTaskAction {
    ...
    public ReopenTaskAction(TaskRepository taskRepository,
    ...
    @Transactional
    public TaskResponse execute(ReopenTaskCommand cmd) {
        ...
        if (current != TaskStatus.DONE && current != TaskStatus.CANCELLED) {
            throw ProjectExceptions.taskCannotTransition(current.name(), TaskStatus.TODO.name());
        }

        Task saved = taskRepository.save(task.reopen());

        platformPublisher.enqueueTask(saved, "TASK_REOPENED");

        activityLogger.logSuccess(
                ProjectEntityTypes.TASK,
                saved.id(),
                ProjectActivityActions.REOPEN_TASK,
                "Task reopened: " + saved.code()
        );
