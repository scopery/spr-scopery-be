...
import com.company.scopery.modules.project.milestone.application.command.ArchiveProjectMilestoneCommand;
...
@Component
public class ArchiveProjectMilestoneAction {
    ...
    public ArchiveProjectMilestoneAction(ProjectMilestoneRepository milestones,
    ...
    @Transactional
    public MilestoneResponse execute(ArchiveProjectMilestoneCommand cmd) {
        authorization.requireGanttMilestoneArchive(cmd.projectId());
        ...
        ProjectMilestone saved = milestones.save(existing.archive(actorId));
        publisher.enqueueMilestone(saved, "PROJECT_MILESTONE_ARCHIVED");
        publisher.auditMilestoneArchived(actorId, project, saved);
        activityLogger.logSuccess(ProjectEntityTypes.PROJECT_MILESTONE, saved.id(),
                ProjectActivityActions.PROJECT_MILESTONE_ARCHIVED, "Milestone archived: " + saved.name());
