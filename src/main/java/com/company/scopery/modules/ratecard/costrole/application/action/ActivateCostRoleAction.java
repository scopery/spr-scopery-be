...
@Component
public class ActivateCostRoleAction {
    ...
    public ActivateCostRoleAction(CostRoleRepository costRoleRepository,
    ...
    @Transactional
    public CostRoleResponse execute(ActivateCostRoleCommand cmd) {
        ...
        CostRole saved = costRoleRepository.save(role.activate());

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_COST_ROLE, saved.id(),
                "COST_ROLE_ACTIVATED", RateCardPlatformPublisher.mapOf("id", saved.id(), "code", saved.code()));
        activityLogger.logSuccess(RateCardEntityTypes.COST_ROLE, saved.id(),
                RateCardActivityActions.COST_ROLE_ACTIVATED, "Cost role activate: " + saved.code());
