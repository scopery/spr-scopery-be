...
import com.company.scopery.modules.ratecard.inflationpolicy.application.command.ArchiveInflationPolicyCommand;
...
@Component
public class ArchiveInflationPolicyAction {
    ...
    public ArchiveInflationPolicyAction(InflationPolicyRepository repository,
    ...
    @Transactional
    public InflationPolicyResponse execute(ArchiveInflationPolicyCommand cmd) {
        InflationPolicy policy = repository.findById(cmd.id())
                .orElseThrow(() -> RateCardExceptions.inflationNotFound(cmd.id()));

        authorizationService.requireInflationArchive(policy.workspaceId());
        if (policy.status() == InflationPolicyStatus.ARCHIVED) throw RateCardExceptions.inflationArchived(cmd.id());
        InflationPolicy saved = repository.save(policy.archive(authorizationService.currentUserId()));

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_INFLATION_POLICY, saved.id(), "INFLATION_POLICY_ARCHIVED",
        ...
        activityLogger.logSuccess(RateCardEntityTypes.INFLATION_POLICY, saved.id(),
                RateCardActivityActions.INFLATION_POLICY_ARCHIVED, "Inflation policy archive: " + saved.code());
