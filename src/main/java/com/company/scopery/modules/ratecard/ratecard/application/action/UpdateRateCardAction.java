...
import com.company.scopery.modules.ratecard.ratecard.application.command.UpdateRateCardCommand;
...
@Component("ratecardUpdateRateCardAction")
public class UpdateRateCardAction {
    ...
    public UpdateRateCardAction(RateCardRepository rateCardRepository,
    ...
    @Transactional
    public RateCardResponse execute(UpdateRateCardCommand cmd) {
        ...
        RateCard saved = rateCardRepository.save(card.update(cmd.name(), cmd.description(), currency));

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_RATE_CARD, saved.id(), "RATE_CARD_UPDATED",
        ...
        activityLogger.logSuccess(RateCardEntityTypes.RATE_CARD, saved.id(),
                RateCardActivityActions.RATE_CARD_UPDATED, "Rate card update: " + saved.code());
