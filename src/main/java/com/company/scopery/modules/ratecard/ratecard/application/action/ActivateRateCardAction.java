...
import com.company.scopery.modules.ratecard.ratecard.application.command.ActivateRateCardCommand;
...
@Component
public class ActivateRateCardAction {
    ...
    public ActivateRateCardAction(RateCardRepository rateCardRepository,
    ...
    @Transactional
    public RateCardResponse execute(ActivateRateCardCommand cmd) {
        ...
        RateCard saved = rateCardRepository.save(card.activate());

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_RATE_CARD, saved.id(), "RATE_CARD_ACTIVATED",
        ...
        activityLogger.logSuccess(RateCardEntityTypes.RATE_CARD, saved.id(),
                RateCardActivityActions.RATE_CARD_ACTIVATED, "Rate card activate: " + saved.code());
