...
@Component
public class DeactivateCostRoleAction {
    ...
    public DeactivateCostRoleAction(CostRoleRepository costRoleRepository,
    ...
    @Transactional
    public CostRoleResponse execute(DeactivateCostRoleCommand cmd) {
        ...
        CostRole saved = costRoleRepository.save(role.deactivate());

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_COST_ROLE, saved.id(),
                "COST_ROLE_DEACTIVATED", RateCardPlatformPublisher.mapOf("id", saved.id(), "code", saved.code()));
        activityLogger.logSuccess(RateCardEntityTypes.COST_ROLE, saved.id(),
                RateCardActivityActions.COST_ROLE_DEACTIVATED, "Cost role deactivate: " + saved.code());
