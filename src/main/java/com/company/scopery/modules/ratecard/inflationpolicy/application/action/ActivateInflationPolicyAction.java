...
import com.company.scopery.modules.ratecard.inflationpolicy.application.command.ActivateInflationPolicyCommand;
...
@Component
public class ActivateInflationPolicyAction {
    ...
    public ActivateInflationPolicyAction(InflationPolicyRepository repository,
    ...
    @Transactional
    public InflationPolicyResponse execute(ActivateInflationPolicyCommand cmd) {
        ...
        InflationPolicy saved = repository.save(policy.activate());

        platformPublisher.enqueue(RateCardPlatformPublisher.AGGREGATE_INFLATION_POLICY, saved.id(), "INFLATION_POLICY_ACTIVATED",
        ...
        activityLogger.logSuccess(RateCardEntityTypes.INFLATION_POLICY, saved.id(),
                RateCardActivityActions.INFLATION_POLICY_ACTIVATED, "Inflation policy activate: " + saved.code());
