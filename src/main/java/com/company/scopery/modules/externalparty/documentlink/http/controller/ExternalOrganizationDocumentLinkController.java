...
@RestController
@RequestMapping(ExternalPartyApiPaths.ORGANIZATION_DOC_LINKS)
@Tag(name = "External Party - Document Links")
public class ExternalOrganizationDocumentLinkController {
    private final CreateExternalPartyDocumentLinkAction create;
    private final ExternalPartyDocumentLinkQueryService query;

    public ExternalOrganizationDocumentLinkController(CreateExternalPartyDocumentLinkAction create, ExternalPartyDocumentLinkQueryService query) {
        this.create = create; this.query = query;
    }

    @PostMapping @Operation(summary = "Link document to organization")
    public ApiResponse<ExternalPartyDocumentLinkResponse> create(@PathVariable UUID workspaceId,
            @PathVariable UUID organizationId, @Valid @RequestBody CreateExternalPartyDocumentLinkRequest r) {
        return ApiResponse.success(create.execute(new CreateExternalPartyDocumentLinkCommand(
                workspaceId, organizationId, null, r.documentId(), r.linkNote())));
    }

    @GetMapping @Operation(summary = "List organization document links")
    public ApiResponse<List<ExternalPartyDocumentLinkResponse>> list(@PathVariable UUID workspaceId, @PathVariable UUID organizationId) {
