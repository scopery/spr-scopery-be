...
@Component
public class CompletePrivacyRequestAction {
    private final PrivacyRequestRepository repo;
    private final TrustAuthorizationService auth;
    public CompletePrivacyRequestAction(PrivacyRequestRepository repo, TrustAuthorizationService auth) {
        this.repo = repo; this.auth = auth;
    }
    @Transactional
    public PrivacyRequestResponse execute(UUID workspaceId, UUID requestId, String resolutionSummary) {
        auth.requireManage(workspaceId);
        var req = repo.findById(requestId).orElseThrow(() -> TrustExceptions.privacyNotFound(requestId));
        try { return PrivacyRequestResponse.from(repo.save(req.complete(resolutionSummary))); }
        catch (IllegalArgumentException e) { throw TrustExceptions.rejectionReasonRequired(); }
