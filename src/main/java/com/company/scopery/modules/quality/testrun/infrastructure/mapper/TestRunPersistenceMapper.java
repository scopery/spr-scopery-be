...
public class TestRunPersistenceMapper {
    public TestRun toDomain(TestRunJpaEntity e) {
        RunScope scope = e.getRunScope() != null ? RunScope.valueOf(e.getRunScope()) : RunScope.FUNCTIONAL;
        return new TestRun(e.getId(), e.getProjectId(), e.getWorkspaceId(), e.getTestPlanId(), e.getTestSuiteId(), e.getReleasePackageId(),
                e.getDeploymentEnvironmentId(), e.getName(), TestRunType.valueOf(e.getRunType()), scope, TestRunStatus.valueOf(e.getStatus()),
    ...
    public TestRunJpaEntity toJpaEntity(TestRun d) {
        ...
        e.setName(d.name()); e.setRunType(d.runType().name()); e.setRunScope(d.runScope() != null ? d.runScope().name() : RunScope.FUNCTIONAL.name());
        e.setStatus(d.status().name()); e.setStartedAt(d.startedAt());
