...
import com.company.scopery.modules.quality.testrun.application.command.CreateTestRunCommand;
...
public class CreateTestRunAction {
    ...
    public CreateTestRunAction(ProjectRepository projects, TestRunRepository repo,
            TestRunMembershipRepository membership, QualityAuthorizationService authorization,
            QualityActivityLogger activityLogger) {
        this.projects=projects; this.repo=repo; this.membership=membership;
    ...
    public TestRunResponse execute(CreateTestRunCommand c) {
        ...
        var saved = repo.save(TestRun.create(project.id(), project.workspaceId(), c.name().trim(),
                QualityEnumParser.parseRequired(TestRunType.class, c.runType(), "runType"),
                QualityEnumParser.parseOptional(RunScope.class, c.runScope(), "runScope"),
                c.testPlanId(), c.testSuiteId(), c.releasePackageId()));

        if (c.caseIds() != null && !c.caseIds().isEmpty()) {
            int order = 0;
            for (var ref : c.caseIds()) {
                if (!membership.exists(saved.id(), ref.caseKind(), ref.caseId())) {
                    membership.save(TestRunMembershipItem.create(saved.projectId(), saved.id(), ref.caseKind(), ref.caseId(), order++));
                }
            }
        }
