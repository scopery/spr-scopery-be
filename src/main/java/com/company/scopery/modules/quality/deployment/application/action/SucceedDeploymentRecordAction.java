...
@Component
public class SucceedDeploymentRecordAction {
        private final DeploymentRecordRepository repo;
    private final QualityAuthorizationService authorization;
    private final QualityActivityLogger activityLogger;
    public SucceedDeploymentRecordAction(DeploymentRecordRepository repo, QualityAuthorizationService authorization, QualityActivityLogger activityLogger) {
    ...
    public DeploymentRecordResponse execute(UUID projectId, UUID id) {
        ...
        if (e.status() != DeploymentStatus.IN_PROGRESS) throw QualityExceptions.deploymentInvalidStatus("Can only succeed IN_PROGRESS");
        var saved = repo.save(e.succeed());
        activityLogger.logSuccess(QualityEntityTypes.DEPLOYMENT_RECORD, saved.id(), QualityActivityActions.DEPLOYMENT_SUCCEEDED, "Deployment succeeded");
