...
@Component
public class ReadyForRetestDefectAction {
    private final DefectRepository repo;
    private final QualityAuthorizationService authorization;
    private final CurrentUserAuthorizationService currentUser;
    private final QualityActivityLogger activityLogger;
    public ReadyForRetestDefectAction(DefectRepository repo, QualityAuthorizationService authorization,
    ...
    public DefectResponse execute(UUID projectId, UUID defectId) {
        authorization.requireDefectResolve(projectId);
        var actor = currentUser.resolveCurrentUser();
        var d = repo.findByIdAndProjectId(defectId, projectId).orElseThrow(() -> QualityExceptions.defectNotFound(defectId));
        var saved = repo.save(d.readyForRetest());
        activityLogger.logSuccess(QualityEntityTypes.DEFECT, saved.id(), QualityActivityActions.DEFECT_FIXED, "Defect ready for retest");
