...
public record TestRun(UUID id, UUID projectId, UUID workspaceId, UUID testPlanId, UUID testSuiteId, UUID releasePackageId,
        UUID deploymentEnvironmentId, String name, TestRunType runType, RunScope runScope, TestRunStatus status, Instant startedAt,
        Instant completedAt, UUID executedBy, String summaryJson, Instant archivedAt, UUID archivedBy, String traceId,
        int version, Instant createdAt, Instant updatedAt) {
    public static TestRun create(UUID projectId, UUID workspaceId, String name, TestRunType type, RunScope runScope, UUID testPlanId, UUID testSuiteId, UUID releasePackageId) {
        Instant now = Instant.now();
        return new TestRun(UUID.randomUUID(), projectId, workspaceId, testPlanId, testSuiteId, releasePackageId, null, name, type,
                runScope != null ? runScope : RunScope.FUNCTIONAL,
                TestRunStatus.PLANNED, null, null, null, null, null, null, null, -1, now, now);
    }
    public TestRun start(UUID actorId) {
        if (status != TestRunStatus.PLANNED) throw new IllegalStateException("must be planned");
        return new TestRun(id, projectId, workspaceId, testPlanId, testSuiteId, releasePackageId, deploymentEnvironmentId, name, runType, runScope,
