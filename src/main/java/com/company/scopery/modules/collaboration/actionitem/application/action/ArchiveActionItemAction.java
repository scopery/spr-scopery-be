...
import com.company.scopery.modules.collaboration.actionitem.application.command.ArchiveActionItemCommand;
...
@Component
public class ArchiveActionItemAction {
    ...
    public ArchiveActionItemAction(MeetingActionItemRepository actions, CollaborationAuthorizationService authorization, CurrentUserAuthorizationService currentUser, CollaborationActivityLogger activityLogger) {
        this.actions=actions; this.authorization=authorization; this.currentUser=currentUser; this.activityLogger=activityLogger;
    }
    @Transactional
    public MeetingActionItemResponse execute(ArchiveActionItemCommand c) {
        ...
        a = actions.save(a.archive(actor.id()));
        activityLogger.logSuccess(CollaborationEntityTypes.ACTION_ITEM, a.id(), CollaborationActivityActions.ACTION_ARCHIVED, "Action archived");
