...
public class RejectSuggestionItemAction {
    ...
    public RejectSuggestionItemAction(AiPlanningSuggestionRepository suggestions,
    ...
    @Transactional
    public AiPlanningSuggestionItemResponse execute(RejectSuggestionItemCommand command) {
        ...
        try {
            item = items.save(item.reject(actor.id()));
        } catch (IllegalStateException ex) {
            throw AiPlanningExceptions.invalidStatus(ex.getMessage());
        }
        reviewActions.save(AiPlanningReviewAction.create(
                command.suggestionId(), command.itemId(), ReviewActionType.REJECT_ITEM, actor.id(), null, MDC.get("traceId")));
        activityLogger.logSuccess(AiPlanningEntityTypes.SUGGESTION_ITEM, item.id(),
                AiPlanningActivityActions.ITEM_REJECTED, "Suggestion item rejected");
