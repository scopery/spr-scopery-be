...
@Component("pfCreateVendorCostAction")
public class CreateVendorCostAction {
    ...
    public CreateVendorCostAction(ProjectRepository projects,
    ...
    public VendorCostResponse execute(CreateVendorCostCommand command) {
        ...
        ProjectVendorCost cost = vendorCosts.save(ProjectVendorCost.create(
                scenario.id(), scenario.projectId(), command.projectPhaseId(),
                command.vendorName(), command.description(), command.amount(), currency));
        recalculator.recalculate(scenario, false);
        UUID actorId = currentUser.resolveCurrentUser().id();
        publisher.enqueueScenario(scenario, "PROJECT_VENDOR_COST_CREATED");
        ...
        activityLogger.logSuccess(ProjectFinanceEntityTypes.VENDOR_COST, cost.id(),
                ProjectFinanceActivityActions.PROJECT_VENDOR_COST_CREATED, "Vendor cost created");
