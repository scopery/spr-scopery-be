...
@Component("pfCreateCustomCostAction")
public class CreateCustomCostAction {
    ...
    public CreateCustomCostAction(ProjectRepository projects,
    ...
    public CustomCostResponse execute(CreateCustomCostCommand command) {
        authorization.requireCostCreate(command.projectId());
        ...
        ProjectCustomCost cost = customCosts.save(ProjectCustomCost.create(
                scenario.id(), scenario.projectId(), command.projectPhaseId(), category,
                command.name(), command.description(), command.amount(), currency, command.costDate()));
        recalculator.recalculate(scenario, false);
        UUID actorId = currentUser.resolveCurrentUser().id();
        publisher.enqueueScenario(scenario, "PROJECT_CUSTOM_COST_CREATED");
        ...
        activityLogger.logSuccess(ProjectFinanceEntityTypes.CUSTOM_COST, cost.id(),
                ProjectFinanceActivityActions.PROJECT_CUSTOM_COST_CREATED, "Custom cost created");
