...
@Component
public class ProjectProfitabilitySummaryPersistenceMapper {

    public ProjectProfitabilitySummary toDomain(ProjectProfitabilitySummaryJpaEntity e) {
        return new ProjectProfitabilitySummary(
                e.getId(),
                e.getWorkspaceId(),
                e.getProjectId(),
                e.getCurrency(),
                e.getBaselineRevenue(),
                e.getForecastRevenue(),
                e.getBaselineCost(),
                e.getForecastCost(),
                e.getForecastProfit(),
                e.getForecastMarginPercent(),
                e.getProfitabilityStatus(),
                e.getLastSnapshotAt(),
    ...
    public ProjectProfitabilitySummaryJpaEntity toJpaEntity(ProjectProfitabilitySummary d) {
        ProjectProfitabilitySummaryJpaEntity e = new ProjectProfitabilitySummaryJpaEntity();
        e.setId(d.id());
        e.setWorkspaceId(d.workspaceId());
        e.setProjectId(d.projectId());
        e.setCurrency(d.currency());
        e.setBaselineRevenue(d.baselineRevenue());
        e.setForecastRevenue(d.forecastRevenue());
        e.setBaselineCost(d.baselineCost());
        e.setForecastCost(d.forecastCost());
        e.setForecastProfit(d.forecastProfit());
        e.setForecastMarginPercent(d.forecastMarginPercent());
        e.setProfitabilityStatus(d.profitabilityStatus());
        e.setLastSnapshotAt(d.lastSnapshotAt());
