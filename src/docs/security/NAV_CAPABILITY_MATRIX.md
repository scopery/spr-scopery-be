...
| D8 | PROJECT IAM resource is live (Phase C): project ops/nav authorize on PROJECT when present; list filters by per-project grant; workspace PROJECT_VIEW remains the Projects-section gate |
...
| Organization directory | `/workspace/{wsId}/organization/directory?tab=members\|invitations\|teams` **or** keep `/organization/members` + `/organization/invitations` but sidebar label = **Organization** |
...
- [x] `IamResourceType.PROJECT` + owner policy + bootstrap on create
- [x] Backfill PROJECT resources + member baselines for workspace members
- [x] Authorize on PROJECT resource when present (no workspace fallback)
- [x] List / My Work filtered by per-project `PROJECT_VIEW`
- [x] NAV_PROJECT with `projectId` evaluates on PROJECT resource
